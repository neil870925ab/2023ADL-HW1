...
def main():
    ...
    def evaluating(eval_dataloader, model):     #add
        all_start_logits = []
        all_end_logits = []
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if args.do_plot:        #add
            num_data_points = 5  
            num_steps = len(active_dataloader)
            step_interval = num_steps // num_data_points
            x_step = []
            y_loss = []
            y_EM = []
            ...
            with accelerator.accumulate(model):
                ...
                if args.do_plot:        #add
                    if step % step_interval == 0:
                        ...
                        model.eval()
                        outputs_numpy = evaluating(eval_dataloader, model)
                        prediction = post_processing_function(eval_examples, eval_dataset, outputs_numpy)
                        eval_metric = compute_em(predictions=prediction.predictions, references=prediction.label_ids)
                        y_EM.append(eval_metric)
                        model.train()
    ...
    # outputs_numpy = (start_logits_concat, end_logits_concat)
    outputs_numpy = evaluating(eval_dataloader, model)       #changed
    prediction = post_processing_function(eval_examples, eval_dataset, outputs_numpy)
