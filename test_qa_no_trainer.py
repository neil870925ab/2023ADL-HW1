...
def parse_args():
    ...
    parser.add_argument("--do_plot", action="store_true", help="To plot the loss and EM for the report")
    parser.add_argument(
...
def main():
    ...
    if args.dataset_name is not None:
        ...
        # extension = args.train_file.split(".")[-1]
        extension = args.test_file.split(".")[-1]       #add
    ...
    # relevant_column_name = "relevant"     #because there is no "relevant" column in the json file
    ...
    #         examples[question_column_name] if pad_on_right else [context[i] for i in examples[relevant_column_name]],
    #         [context[i] for i in examples[relevant_column_name]] if pad_on_right else examples[question_column_name],
    ...
    def prepare_validation_features(examples):
        ...
        tokenized_examples = tokenizer(
            # examples[question_column_name] if pad_on_right else [context[i] for i in examples[relevant_column_name]],
            # [context[i] for i in examples[relevant_column_name]] if pad_on_right else examples[question_column_name],
            examples[question_column_name] if pad_on_right else [context for context in examples[relevant_column_name]],
            [context for context in examples[context_column_name]] if pad_on_right else examples[question_column_name],
            truncation="only_second" if pad_on_right else "only_first",
            max_length=max_seq_length,
            stride=args.doc_stride,
            return_overflowing_tokens=True,
    ...
    def post_processing_function(examples, features, predictions, stage="eval"):
        ...
        formatted_predictions = [{"id": k, "answers": v} for k, v in predictions.items()]   #add
        references = {}                
        # references = [{"id": ex["id"], "answers": ex["answer"]["text"]} for ex in examples]
        references = [{"id": ex["id"], "answers": ex[answer_column_name]} for ex in examples]

        # print('formatted_predictions[0:3]', formatted_predictions[0:3])
        # print('references[0:3]', references[0:3])
    ...
    # def compute_em(predictions, references):    #add
    #     if(len(predictions)!=len(references)):
    #         raise ValueError('the two length is not the same')
    #     if(type(predictions)!=type(references)):
    #         raise ValueError('the two type is not the same')

    #     count = 0
    #     for dict_p in predictions:
    #         for dict_r in references:
    #             if(set(dict_p.items())==set(dict_r.items())):
    #                 count += 1
    #     em_score = count/len(references)
    ...
    # eval_metric = compute_em(predictions=prediction.predictions, references=prediction.label_ids) #add
    ...
    if args.do_predict:
        ...
        predict_dataloader = accelerator.prepare(predict_dataloader)  #add
    ...
    # with open(os.path.join(args.output_dir, args.output_file), "w", encoding = 'UTF-8-SIG') as f:
    with open(args.output_file, "w", encoding = 'UTF-8-SIG') as f:
        writer = csv.writer(f)
        writer.writerow(["id", "answer"])
        for p in prediction.predictions:
            writer.writerow([p["id"], p["answers"]])
